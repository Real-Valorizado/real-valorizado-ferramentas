...
export default function Text(props: FieldInputProps) {
  ...
  function increase() {
    ...
    if (newCount > bounds.max) {
      setError(`Coloque um valor entre ${bounds.min} e ${bounds.max}`);
    ...
    <Fragment>
      <div className='w-auto inline-block'>
        <div className='flex h-12'>
          {!visible && (
            <Fragment>
              <strong
                ...
                className='font-bold bg-blue-50 text-3xl text-slate-800 inline-block underline cursor-pointer'
              >
                {text}
              </strong>
              {showArrows && (
                <div className='flex flex-col justify-center'>
                  <ChevronUpIcon
                    onClick={increase}
                    className='h-5 w-5 inline-block text-blue-500 cursor-pointer'
                  />
                  <ChevronDownIcon
                    onClick={decrease}
                    className='h-5 w-5 inline-block text-blue-500 cursor-pointer'
          ...
          {visible && (
            <input
              className='font-bold text-3xl text-blue-700 w-48 shadow-sm'
              type="text"
              value={count}
              autoFocus
